...
סוכן יצירת המשימות פועל באותו מסלול Spark: הוא מחזיר JSON מוגבל, והשמות מותאמים מקומית רק לרשומות שהמשתמש כבר מורשה לראות. אל Gemini נשלחים רק הקשרים מוסדיים מצומצמים ללא שמות או מזהי אנשי צוות. משימות קודמות שהמשתמש יצר משמשות כזיכרון אישי מפוצל, וכללים ותבניות מוסדיים משפיעים על כלל הצוות רק לאחר שמנהל שמר אותם. הסוכן אינו כותב משימה למסד; הטופס המאוחד נשמר רק לאחר אישור מפורש ובאמצעות שירות המשימות הקיים. אפשר לכבות אותו או לשנות את המודל ללא שינוי קוד באמצעות Remote Config: `zoko_task_assistant_enabled`, `zoko_ai_model`, `zoko_task_assistant_daily_limit`, `zoko_task_assistant_timeout_ms`. אם AI Logic או Remote Config אינם זמינים, המנוע המקומי עדיין מחזיר הצעה והיצירה הידנית ממשיכה לעבוד.
