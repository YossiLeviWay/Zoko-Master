...
import {
  AlertCircle, BookOpen, CheckCircle2, Circle, Clock3, Edit3, FileUser,
...
export default function StudentProfile({
  ...
  personalFileAccess, cvAccess, onClose, onEdit,
}) {
  ...
  return <div className="modal-overlay" onClick={onClose}><div className="modal-content modal-content--wide student-profile-modal" role="dialog" aria-modal="true" aria-label={`פרופיל ${student.fullName}`} onClick={event => event.stopPropagation()}>
    ...
    <div className="modal-form student-profile-content">
      ...
      {activeTab === 'study' && <section className="student-profile-section"><h4 className="student-profile-section-title">התקדמות במסלול הלימודים</h4>{requirements.length === 0 ? <div className="student-empty-state"><BookOpen size={26} /><strong>אין דרישות מסלול מוגדרות</strong><p>המגמות והשנתונים נשמרים ברשומות הלימודים השנתיות.</p></div> : <div className="req-checklist">{requirements.map(requirement => { const state = requirementStatus[requirement.id] || 'pending'; return <button type="button" key={`${requirement.trackName}_${requirement.id}`} className={`req-checklist-item req-status--${state} ${canEdit ? 'req-checklist-item--clickable' : ''}`} onClick={() => toggleRequirement(requirement.id)} disabled={!canEdit}><span>{statusIcon(state)}</span><span className="req-checklist-name">{requirement.name}<small>{requirement.trackName}</small></span><span className="req-status-label">{STATUS_OPTIONS.find(item => item.id === state)?.label}</span></button>; })}</div>}</section>}
